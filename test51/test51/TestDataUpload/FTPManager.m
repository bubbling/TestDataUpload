...
//  FTPManager
//
//  Modify by Xiong liping on 2015.11.21 email:liping_xiong@163.com
...
// ***********
//
// ** 1.3 (2015-11-19) by xiong liping
//     - Methods added:
//         - (BOOL) createFloders:
//
        ...
        do {
            ...
            if (bytesConsumed > 0) {
                if (thisEntry != NULL) {
                    ...
                    NSString *nameEntry = entry[(id)kCFFTPResourceName];
                    if (nameEntry!=nil) {
                        entry[nameEntry] = [[NSString alloc] initWithData:[nameEntry dataUsingEncoding:NSMacOSRomanStringEncoding allowLossyConversion:YES]
...
- (BOOL) createNewFolders:(NSString*)foldersPath atServer:(FMServer*)server
{
    if (![self _checkFMServer:server]) {
        return NO;
    }
    ...
    if ([foldersPath isEqualToString:@""]) {
        return NO;
    }
    ...
    NSArray* arrFolder=[foldersPath componentsSeparatedByString:@"/"];
    if (arrFolder==nil) {
        arrFolder=[foldersPath componentsSeparatedByString:@"\\"];
    }
    for (int i=0; i<[arrFolder count]; i++) {
        if (![arrFolder[i] isEqualToString:@""]) {
            strPath=[strPath stringByAppendingPathComponent:arrFolder[i]];
            createRes=RunInSeparateThread([self _createNewFolder:strPath atServer:server]);
        }
    }
    return createRes;
}
